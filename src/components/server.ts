...
app.get("/api/user", async (req, res) => {
    const userId = req.query.userId || "rider_101";
    const user = await UserModel.findOne({ id: userId });
    res.json({ user });
});

app.post("/api/user", async (req, res) => {
    const userId = req.body.id || "rider_101";
    const user = await UserModel.findOneAndUpdate({ id: userId }, req.body, { upsert: true, returnDocument: 'after' });
...
app.post("/api/rides/book", async (req, res) => {
    const { pickup, drop, rideType, fare, breakdown, targetDriverId, riderId } = req.body;
    const currentRiderId = riderId || "rider_101";
    const activeRide = await RideModel.findOne({ riderId: currentRiderId, status: { $in: ['searching', 'accepted', 'arrived', 'in_progress'] } });
...
app.get("/api/rides/active", async (req, res) => {
    const userId = req.query.userId || "rider_101";
    const activeRide = await RideModel.findOne({ riderId: userId, status: { $in: ['searching', 'accepted', 'arrived', 'in_progress'] } });
...
app.post("/api/rides/cancel", async (req, res) => {
    const userId = req.body.userId || "rider_101";
    const activeRide = await RideModel.findOne({ riderId: userId, status: { $in: ['searching', 'accepted', 'arrived'] } });
    if (activeRide) {
        activeRide.status = "cancelled";
        await activeRide.save();
...
app.post("/api/rides/verify-otp", async (req, res) => {
    const userId = req.body.userId || "rider_101";
    const activeRide = await RideModel.findOne({ riderId: userId, status: "arrived", id: req.body.rideId });
...
app.get("/api/rider/profile/:userId", async (req, res) => {
    const user = await UserModel.findOne({ id: req.params.userId });
    res.json({ exists: !!user && !!user.phone, user });
});
...
app.post("/api/rider/profile", async (req, res) => {
    ...
    const user = await UserModel.findOneAndUpdate(
        { id: userId },
        { name, phone, verified: true, role: 'rider' },
